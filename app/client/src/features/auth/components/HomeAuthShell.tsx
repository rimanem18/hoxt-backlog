...
/**
 * ホームページの認証状態に応じた表示を制御するコンポーネント
 *
 * 認証状態に関係なくログイン促進メッセージを表示。
 * 認証済みユーザーには自動的にダッシュボードへリダイレクト。
 */
export function HomeAuthShell(props: HomeAuthShellProps): React.ReactNode {
  const { isAuthenticated, user, authError, isAuthRestoring } = useAppSelector(
    (state) => state.auth,
  );
  const { navigate } = useHomeShellServices();
  ...
  useEffect(() => {
    if (!isAuthRestoring && isAuthenticated && user) {
      ...
      navigate('/dashboard'); // 履歴を残さずリダイレクト
    }
  }, [isAuthRestoring, isAuthenticated, user, navigate]);
