 ...
 * @returns マッチャー結果
 */
export function toFailWithError(received: Promise<any>, errorType: keyof typeof ERROR_MAPPINGS) {
 ...
 * @returns マッチャー結果
 */
export function toFailWithMessage(received: Promise<any>, expectedMessage: string) {
 ...
 * @returns マッチャー結果
 */
export function toCompleteWithinTimeLimit(executionTime: number, limit: number) {
...
export function toHaveLoggedMessage(
  ...
  expectedMeta?: Record<string, any>
) {
  ...
  const matchingCall = calls.find((call: any[]) => {
    ...
    if (expectedMeta) {
      return Object.keys(expectedMeta).every(key => 
        meta && meta[key] === expectedMeta[key]
      );
    }
...
export function toHaveUserProperties(
  ...
  return Object.keys(expectedProperties).every(key => {
    return expect(user[key]).toBe(expectedProperties[key as keyof typeof expectedProperties]);
  });
}
...
export const MockHelper = {
   ...
   */
  toHaveBeenCalledWithArgs(mockFn: any, ...expectedArgs: any[]) {
   ...
   */
  toHaveBeenCalledTimes(mockFn: any, times: number) {
   ...
   * @param mockFn モック関数
   */
  notToHaveBeenCalled(mockFn: any) {
