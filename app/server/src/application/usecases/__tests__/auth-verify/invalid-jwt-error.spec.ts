...
describe('AuthenticateUserUseCase - 無効JWT検証エラーテスト', () => {
  ...
  test('無効な署名を持つJWTで認証が失敗する', async () => {
    ...
    ).rejects.toThrow('認証トークンが無効です'); // 【確認内容】: JWT署名検証失敗時に適切な例外が発生することを確認 🟢
    ...
    try {
      ...
      expect(errorObj.name).toBe('AuthenticationError'); // 【確認内容】: 正しいドメインエラータイプが発生することを確認 🟢
      expect(errorObj.code).toBe('INVALID_TOKEN'); // 【確認内容】: 適切なエラーコードが設定されることを確認 🟢
      expect(errorObj.message).toBe('認証トークンが無効です'); // 【確認内容】: ユーザーフレンドリーなエラーメッセージが提供されることを確認 🟢
    }
