...
describe('POST /api/auth/verify 統合テスト', () => {
  ...
  test('POST /api/auth/verify で有効JWTによる認証が成功すること', async () => {
    // 【テスト目的】: 実際のHTTPリクエストでエンドツーエンドの認証処理が正常動作することを確認
    // 【テスト内容】: 実際のHTTPリクエストを送信して、ルーティング→AuthController→レスポンスの統合フローを検証
    // 【期待される動作】: HTTPリクエスト受信からJWT検証・ユーザー認証・JSONレスポンス返却までが正常実行される
    // 🟢 信頼性レベル: 要件定義書とgreetRoutes.tsの実装パターンで確認済み

    // 【統合テストデータ準備】: 実際にフロントエンドから送信される想定のHTTPリクエスト形式でテストデータを設定
    // 【初期条件設定】: 有効なJWTトークンと適切なHTTPヘッダーを含むリクエストを準備
    const httpRequest = {
      method: 'POST',
      url: '/api/auth/verify',
    ...
    // 【実HTTP通信実行】: 実際のHTTPリクエストをサーバーに送信し、統合フローを実行
    // 【統合フロー確認】: ルーティング→AuthController→UseCaseの完全な処理フロー実行
  ...
  test('依存関係の注入が失敗した場合に適切なサーバーエラーが返されること', async () => {
    ...
    // 【確認項目】: サーバーが応答することを確認（依存性正常時は200、エラー時は500） 🔴
