...
2. Opus Plan エージェントで計画または計画に不足する情報の明示をメインエージェントに返却。不足情報があれば 1 へ。情報が十分であれば計画を済ませて 3 へ。（1 へ戻ってよいのは2回までとする）
...
5. 4と並行してレビューを依頼。レビュー結果が返ってきたら、 resolve-feedback スキルを使用
