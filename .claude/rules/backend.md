...
- **必須**: ドメインルートファイル（`xxxRoutes.ts`）のモジュールトップレベルでDIコンテナからUseCaseを即時解決する既存パターンに従う場合、そのドメインがDB接続以外の新しい必須環境変数（外部サービス等）に依存するなら、遅延評価プロキシを挟みモジュールimport時にDIコンテナへ触れないようにする（未設定の環境変数によって、そのドメインを一度も呼び出していなくてもサーバー全体の起動が失敗することを防ぐため）
