...
- 既存の「モジュールトップレベルでDIコンテナを即時評価する」パターンは、依存する環境変数がすでにサーバー起動の必須要件になっている（`validateEnv()`等でチェック済み）場合にのみ安全。新しいドメインが新しい外部サービス依存を追加する際は、そのUseCaseがまだ本番運用の前提（IaC・環境変数設定）を満たしていない可能性を考慮し、DI解決を遅延させるか、必須環境変数を`validateEnv()`に統合するかを検討する必要がある
