...
- **実装例**:
  ...
  ```
- **テスト要件**:
  - [x] 単体テスト: APIクライアントが型安全に動作
  - [x] 単体テスト: React Queryフックが正しい型を返す
  - [x] TypeScriptコンパイルで型エラーが検出される
- **完了条件**:
  - [x] APIクライアントが完全に型安全
  - [x] 全テストが成功する
...
- **テスト要件**:
  - [x] 統合テスト: 既存の認証フローが動作
  - [x] 統合テスト: ユーザープロフィール取得が動作
  - [x] E2Eテスト: ログイン→プロフィール表示フロー
- **完了条件**:
  - [x] 既存のAPI呼び出しが型安全なクライアントに置き換わっている
  - [x] 全テストが成功する
- **実装成果**:
  - `app/client/src/lib/api.ts`: `openapi-fetch`を使用した型安全なAPIクライアント
  - `app/client/src/lib/api.test.ts`: APIクライアントのテストコード
  - ⚠️ 一部従来のfetch使用箇所が残存（`userService.ts`）
...
- **実装例**:
  ...
  ```
- **テスト要件**:
  - [x] 単体テスト: バリデーション成功時は正常処理
  - [x] 単体テスト: バリデーション失敗時は400エラー
  - [x] パフォーマンステスト: レスポンスタイムへの影響を測定
- **エラーハンドリング**:
  - [x] 詳細なフィールドエラーメッセージ
  - [x] クライアントに内部エラーを露出しない
- **完了条件**:
  - [x] リクエストバリデーションが全エンドポイントで動作
  - [x] パフォーマンス要件を満たす
- **実装成果**:
  - `app/server/src/presentation/http/routes/authRoutes.ts:20-52`: `defaultHook`によるZodバリデーション
  - `app/server/src/presentation/http/routes/userRoutes.ts:73-102`: 同様のバリデーション実装
  - フィールド単位の詳細エラーマップ生成機能
...
- **実装例**:
  ...
  ```
- **完了条件**:
  - [x] エラーメッセージが開発者にわかりやすい
  - [x] フィールド単位でエラー詳細を提供
- **実装成果**:
  - `authRoutes.ts:32-51`: フィールド単位のエラーマップ生成
  - `userRoutes.ts:83-100`: 同様の実装
  - `result.error.issues.reduce()`によるフィールド単位のエラー詳細生成
