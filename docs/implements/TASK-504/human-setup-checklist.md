...
- [ ] **最小権限**: API トークンは Pages Edit のみ
- [ ] **DNS 権限除外**: DNS Edit 権限は付与しない
- [ ] **トークンローテーション**: 90日以内に再生成予定を設定
- [ ] **環境分離**: production/preview 環境を明確に分離
- [ ] **承認フロー**: 本番デプロイに承認者を設定
...
### 最終確認項目
- [ ] CloudFlare API トークンが正常に動作する
- [ ] Direct Upload プロジェクトが作成済み
- [ ] カスタムドメインの CNAME レコードが設定済み（使用時のみ）
